                ...
                case "/add":
                    if (words.length >= 3) {
                        ...
                        try {
                            double price = Double.parseDouble(words[2]);
                            createProduct(name, price);
                            sendMessage(chatId, "Mahsulot muvaffaqiyatli qo'shildi.");
                        } catch (NumberFormatException e) {
                ...
                case "/delete":
                    if (words.length >= 2) {
                        try {
                            int id = Integer.parseInt(words[1]);
                            deleteProduct(id);
                            sendMessage(chatId, "Mahsulot muvaffaqiyatli o'chirildi.");
                        } catch (NumberFormatException e) {
                            sendMessage(chatId, "ID noto'g'ri formatda. IDni son shaklida kiriting.");
                        }
                    } else {
                        sendMessage(chatId, "Iltimos, mahsulot ID ni kiriting. Masalan: /delete 1");
                    }
                    break;

                default:
                    sendMessage(chatId, "Nimadir xato bo'ldi! Foydalanish: /list, /add, /update yoki /delete [parametrlar]");
                    break;
            }
        }
    }
